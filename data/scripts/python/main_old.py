...
def process_ingredients(logger):
    ...
    try:
        ...
        logger.info(f"✅ Ingredients processed successfully")
        logger.info(f"   Output: ./dataset/ingredients_exploded.csv")
        logger.info(f"   Total records: {len(df_final)}")
        return df_final
    except Exception as e:
        logger.error(f"Ingredient processing failed: {e}", exc_info=True)
        raise
